...
Важно: это соглашение **для клиентских приложений**. Сам `olcrtc` такой файл не читает как входной конфиг, но серверный режим может отдавать подписку по HTTP, если в JSON-конфиге задан верхнеуровневый `port` или CLI-флаг `-port`.
...
| `#refresh:` | Когда клиенту нужно обновить подписку. Для статичных файлов это может быть интервал `5s`, `10m`, `6h`; сервер `olcrtc` пишет Unix time следующего обновления |
...
## Серверная генерация `olcrtc`

Если сервер запущен с `-port <port>` или с верхнеуровневым `"port"` в JSON-конфиге, он начинает отдавать подписку по HTTP:
...
HTTP-ответ имеет `Content-Type: text/plain; charset=utf-8`. Поле `port` в ответ не попадает.

Сервер автоматически пишет:

- `#update:` Unix time последнего обновления данных подписки. Значение обновляется при создании store и при каждом пересоздании room id.
- `#refresh:` если хотя бы у одной локации задан `lifetime`, сервер пишет Unix time следующего обновления: `update + min(lifetime)`. При разных `lifetime` берётся минимальный положительный, чтобы клиент обновился до ближайшей ротации комнаты.

Если `lifetime` нигде не задан, сервер отдаёт явно заданное верхнеуровневое `refresh`, если оно есть.

Локальные поля `name`, `color`, `icon`, `used`, `available`, `ip`, `comment`, `mimo` читаются из каждой location и пробрасываются в подписку. Верхнеуровневые `name`, `color`, `icon`, `used`, `available`, `refresh` относятся ко всей подписке.
...
- Для статичных подписок в `#refresh:` можно использовать короткие интервалы вида `5s`, `10m`, `6h`, `1d`.
- Для подписок, которые отдаёт `olcrtc`, `#refresh:` это Unix time следующего обновления.
