...
### Автоматически менять комнаты

На сервере можно добавить `-lifetime <секунды>`. Тогда сервер будет создавать новую комнату через заданный интервал, отправлять клиенту служебное сообщение и переподключаться в новую комнату:
...
Клиенту отдельный флаг для ротации не нужен - он слушает служебные сообщения сервера и сам переподключается. Во время смены комнаты уже открытые TCP-соединения через SOCKS могут оборваться, приложения должны уметь повторить запрос.
